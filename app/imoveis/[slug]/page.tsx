...
import { Card, CardContent } from "@/components/ui/card" // Removido CardFooter se não usado diretamente aqui
...
async function getPropertyData(slug: string) {
  try {
    ...
    const similarProperties = (allProperties || [])
      .filter(p => property && p._id !== property._id) 
      .slice(0, 6);
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="relative aspect-video overflow-hidden rounded-lg">
                <img
                  ...
                  alt={property.title || "Imagem principal do imóvel"}
              ...
              <div className="grid grid-cols-2 gap-4">
                {property.youtubeVideo && (
                  <div className="relative aspect-[9/16] overflow-hidden rounded-lg col-span-1">
                    <iframe
                      src={`https://www.youtube.com/embed/${property.youtubeVideo.split('v=')[1]?.split('&')[0] || property.youtubeVideo.split('/').pop()}?rel=0&modestbranding=1`} // Corrigido e melhorado para extrair ID do YouTube
            ...
            <div className="grid gap-8 lg:grid-cols-3">
              <div className="lg:col-span-2">
                ...
                <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
                  <div className="rounded-lg border bg-white p-4 text-center"> <Maximize className="mx-auto mb-2 h-5 w-5 text-orange-500" /> <div className="text-sm text-slate-600">Área</div> <div className="font-bold text-slate-900">{property.area}</div> </div>
                  <div className="rounded-lg border bg-white p-4 text-center"> <Bed className="mx-auto mb-2 h-5 w-5 text-orange-500" /> <div className="text-sm text-slate-600">Quartos</div> <div className="font-bold text-slate-900">{property.bedrooms}</div> </div>
                  <div className="rounded-lg border bg-white p-4 text-center"> <Bath className="mx-auto mb-2 h-5 w-5 text-orange-500" /> <div className="text-sm text-slate-600">Banheiros</div> <div className="font-bold text-slate-900">{property.bathrooms}</div> </div>
                  <div className="rounded-lg border bg-white p-4 text-center"> <Car className="mx-auto mb-2 h-5 w-5 text-orange-500" /> <div className="text-sm text-slate-600">Vagas</div> <div className="font-bold text-slate-900">{property.parkingSpots}</div> </div>
                </div>
                ...
                <Tabs defaultValue="description" className="mb-8">
                  ...
                  <TabsContent value="location" className="mt-4 rounded-lg border p-6">
                    ...
                    <div className="mb-4 text-slate-600">
                      ...
                      <p className="mt-2"> Excelente localização com fácil acesso a transporte público, comércio, restaurantes e áreas de lazer. </p>
                    </div>
                    {property.mapUrl ? (
                      <div className="space-y-4">
                        ...
                        <div className="flex gap-2">
                          <a href={property.mapUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center rounded-md bg-orange-500 px-4 py-2 text-sm font-medium text-white hover:bg-orange-600" > <MapPin className="mr-2 h-4 w-4" /> Ver no Google Maps </a>
                          {property.virtualTour && (
                            <a href={property.virtualTour} target="_blank" rel="noopener noreferrer" className="inline-flex items-center rounded-md border border-orange-500 px-4 py-2 text-sm font-medium text-orange-500 hover:bg-orange-50" > <Maximize className="mr-2 h-4 w-4" /> Tour Virtual 360° </a>
                          )}
                        </div>
                      </div>
                    ) : (
                      <div className="aspect-video overflow-hidden rounded-lg bg-slate-200"> <div className="flex h-full items-center justify-center"> <p className="text-slate-500">Mapa da localização indisponível</p> </div> </div>
                    )}
                  </TabsContent>
                </Tabs>
                              ...
                              <Card className="overflow-hidden">
                                <div className="aspect-video w-full overflow-hidden">
                                  <img
                                    ...
                                    alt={similarProperty.title || "Imagem do Imóvel Similar"}
                                ...
                                <CardContent className="p-4">
                                  ...
                                  <div className="mt-2 flex items-center justify-between">
                                    {/* PREÇO DOS IMÓVEIS SIMILARES FORMATADO */}
                ...
                <div className="sticky top-24 space-y-6">
                  <div className="rounded-lg border bg-white p-6 shadow-sm">
                    <div className="mb-4 text-center">
                      ...
                      {/* PREÇO PRINCIPAL DO IMÓVEL FORMATADO */}
                      <div className="text-3xl font-bold text-orange-500">{formatCurrency(property.price)}</div>
                    </div>
                    {(property.condoFee || property.tax) && (
                      <div className="mb-6 border-t border-b py-4">
                        <div className="grid grid-cols-2 gap-4">
                          {property.condoFee && ( <div className="text-center"> <div className="text-sm text-slate-600">Condomínio</div> <div className="font-medium text-slate-900">{formatCurrency(property.condoFee)}</div> </div> )}
                          {property.tax && ( <div className="text-center"> <div className="text-sm text-slate-600">IPTU</div> <div className="font-medium text-slate-900">{formatCurrency(property.tax)}</div> </div> )}
                        </div>
                      </div>
                    )}
                    <div className="space-y-4">
                  ...
                  <div className="rounded-lg border bg-white p-6 shadow-sm">
                    ...
                    <div className="mb-6 flex items-center gap-4">
                      <div className="h-16 w-16 overflow-hidden rounded-full">
                        <img
                          src={getImageUrl(siteSettings?.aboutImage, 200, 200) || "/placeholder.svg"}
                    ...
                    <form className="space-y-4">
                      <div className="space-y-2"> <label htmlFor="nameDetailPg" className="text-sm font-medium text-slate-700"> Nome </label> <input id="nameDetailPg" type="text" className="w-full rounded-md border border-slate-300 p-2 text-sm focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500" placeholder="Seu nome completo" /> </div>
                      <div className="space-y-2"> <label htmlFor="emailDetailPg" className="text-sm font-medium text-slate-700"> Email </label> <input id="emailDetailPg" type="email" className="w-full rounded-md border border-slate-300 p-2 text-sm focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500" placeholder="seu@email.com" /> </div>
                      <div className="space-y-2"> <label htmlFor="phoneDetailPg" className="text-sm font-medium text-slate-700"> Telefone </label> <input id="phoneDetailPg" type="tel" className="w-full rounded-md border border-slate-300 p-2 text-sm focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500" placeholder="(00) 00000-0000" /> </div>
                      <div className="space-y-2"> <label htmlFor="messageDetailPg" className="text-sm font-medium text-slate-700"> Mensagem </label> <textarea id="messageDetailPg" rows={4} className="w-full rounded-md border border-slate-300 p-2 text-sm focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500" placeholder="Olá, tenho interesse neste imóvel e gostaria de mais informações." ></textarea> </div>
      ...
      <footer className="border-t bg-white py-12">
        <div className="container">
          <div className="grid gap-8 md:grid-cols-3">
            <div>
              {siteSettings?.logo && (
                <img
                  ...
                  className="h-14 w-auto mb-4" // Tamanho da logo no rodapé (ajuste se necessário)
                />
              )}
            ...
            <div>
              ...
              <nav className="mt-4 flex flex-col space-y-2">
                <Link href="/" className="text-slate-600 hover:text-orange-500"> Início </Link>
                <Link href="/imoveis" className="text-slate-600 hover:text-orange-500"> Imóveis </Link>
                <Link href="/#sobre" className="text-slate-600 hover:text-orange-500"> Sobre </Link>
                <Link href="/#contato" className="text-slate-600 hover:text-orange-500"> Contato </Link>
              </nav>
            </div>
            <div>
              <h3 className="text-lg font-bold text-slate-900">Fale Conosco</h3>
              <div className="mt-4 space-y-2">
                {siteSettings?.phone && ( <p className="flex items-center text-slate-600"> <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-2 h-5 w-5 text-orange-500" > <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81 .7A2 2 0 0 1 22 16.92z" /> </svg> {siteSettings.phone} </p> )}
                {siteSettings?.email && ( <p className="flex items-center text-slate-600"> <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-2 h-5 w-5 text-orange-500" > <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" /> <polyline points="22,6 12,13 2,6" /> </svg> {siteSettings.email} </p> )}
                {siteSettings?.address && ( <p className="flex items-center text-slate-600"> <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-2 h-5 w-5 text-orange-500" > <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /> <circle cx="12" cy="10" r="3" /> </svg> {siteSettings.address} </p> )}
              </div>
              {siteSettings?.whatsapp && (
                <div className="mt-6">
                  <Link href={whatsappLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center rounded-full bg-green-500 px-4 py-2 text-sm font-medium text-white hover:bg-green-600" >
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-2 h-4 w-4" > <path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z" /> </svg>
                    Fale pelo WhatsApp
                  </Link>
                </div>
              )}
          ...
          <div className="mt-12 border-t pt-6 text-center text-sm text-slate-500">
            <p> © {new Date().getFullYear()} {siteSettings?.title || "Marcelo Victor Imóveis"}. Todos os direitos reservados. Feito pela ALX Mídias. </p>
          </div>
        </div>
      </footer>
    </div>
